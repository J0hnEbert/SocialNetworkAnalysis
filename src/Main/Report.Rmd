---
title: "Social Network Analysis Report on Wine Review Data"
output: 
...
wineproduction_data_2023 <- read.csv("/Users/john-thomas/Desktop/UNI/SNA/wine-producing-countries-2023.csv")
...
adj_EU <- tibble()  # Initialize an empty tibble
...
adj_NA <- tibble()  # Initialize an empty tibble
...
adj_AF <- tibble()  # Initialize an empty tibble
...
while (i >= 1) {  # Adjusted loop condition to avoid i = 0
  ...
  adj <- ifelse(
    ifelse(df_AF_wines_by_country[z, 2] > df_AF_wines_by_country[i, 2],
           ...
           df_AF_wines_by_country[i, 2] / df_AF_wines_by_country[z, 2]) > 10000,
    NA,
    ifelse(df_AF_wines_by_country[z, 2] > df_AF_wines_by_country[i, 2],
           df_AF_wines_by_country[z, 2] / df_AF_wines_by_country[i, 2],
...
adj_OZ <- tibble()  # Initialize an empty tibble
...
plot(simplify(graph_OZ), edge.arrow.size = 0.2,layout = layout, main = "Ociania")

layout = layout_with_graphopt(graph_AF)
plot(simplify(graph_AF), edge.arrow.size = 0.2,layout = layout, main = "Africa")

layout = layout_with_graphopt(graph_AS)
plot(simplify(graph_AS), edge.arrow.size = 0.2,layout = layout, main = "Asia")

layout = layout_with_graphopt(graph_NA)
plot(simplify(graph_NA), edge.arrow.size = 0.2,layout = layout, main = "North America")

layout = layout_with_graphopt(graph_EU)
plot(simplify(graph_EU), edge.arrow.size = 0.2,layout = layout, main = "Europe")

layout = layout_with_graphopt(graph_SA)
plot(simplify(graph_SA), edge.arrow.size = 0.2,layout = layout, main = "South America")
```
